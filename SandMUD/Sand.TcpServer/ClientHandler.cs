...
namespace Sand.TcpServer
{
    ...
    internal sealed class ClientHandler
    {
        ...
        private readonly ConcurrentQueue<byte[]> _dataToSendQueue;
        private readonly int _id;
        private readonly Action<CallbackMessage> _serverCallback;
        ...
        public ClientHandler(TcpClient client, Action<CallbackMessage> serverCallback)
        {
            ...
            //The server callback method - the only way the handler interfaces back with the server (this makes the handler independent from the server)
            _dataToSendQueue = new ConcurrentQueue<byte[]>();
            //This queue handles incoming data to SEND - there's no immediate data sending in the handler

            var handlerThread = new Thread(HandlerLoop) {Priority = ThreadPriority.Lowest};
            //This thread is very low priority because it only handles a single connection
            handlerThread.Start();

            _lastCheck = DateTime.Now;
            //Set the lastCheck to now (this is responsible for pinging when there's no connection activity)
        ...
        private void HandlerLoop()
        {
            ...
            while (_client.Connected)
            {
                ...
                if (_client.Available > 0)
                {
                    ...
                    var bytesReceived = stream.Read(_buffer, 0, _buffer.Length);
                ...
                {
                    byte[] dataToSend;
                    if (_dataToSendQueue.TryDequeue(out dataToSend))
                    {
            ...
            //We're not connected anymore, send a disconnect message via callback
        }
